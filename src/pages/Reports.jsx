...
import { getReports, createReport, updateReport, deleteReport, exportReport } from '../services/api';
import { downloadReportFile } from '../utils/reportBuilder';
...
export default function Reports({ showToast }) {
  ...
  const handleDownload = async (r) => {
    try {
      ...
      const payload = await exportReport(r.id);
      downloadReportFile(payload);
      showToast(`${r.name} downloaded.`);
    } catch (err) {
      showToast(err.message || 'Could not generate report.');
    }
  };
